...
const EMPTY_FORM: Omit<Product, '_id'> = {
  nombre: '', categoria: CATEGORIES[0], precio: 0,
...
const fmtDate = (d: string) =>
  new Date(d).toLocaleString('es-HN', { dateStyle: 'short', timeStyle: 'short' });
...
function getApiKey(): string {
  ...
  return sessionStorage.getItem('dg_api_key') ?? '';
}
  ...
  return (
    <div className={styles.apiKeyPrompt}>
      <p className={styles.apiKeyText}>
        Ingresa tu <code>ADMIN_API_KEY</code> para gestionar productos:
      </p>
      <div className={styles.apiKeyRow}>
        <input className={styles.input} type="password" placeholder="sk-…"
          value={val} onChange={(e) => setVal(e.target.value)} />
        <button className={styles.btnPrimary}
          onClick={() => { sessionStorage.setItem('dg_api_key', val); onSet(val); }}>
          Confirmar
        </button>
      </div>
    </div>
...
function ProductForm({ initial, onSave, onClose }: ProductFormProps) {
  ...
  async function handleSave() {
    if (!form.nombre.trim() || !form.categoria || !form.precio) {
    ...
    <div className={styles.modalOverlay} onClick={onClose}>
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={styles.modalBody}>
          ...
          <div className={styles.grid2}>
            <Field label="Estado / Badge">
              <select className={styles.input} value={form.estado ?? ''} onChange={(e) => set('estado', (e.target.value || null) as Product['estado'])}>
                ...
                <option value="best">Destacado</option>
              </select>
            </Field>
          ...
          <Field label="Highlights (uno por línea)"><textarea className={`${styles.input} ${styles.textarea}`} rows={3} value={highlightsText} onChange={(e) => setHighlightsText(e.target.value)} /></Field>
...
function OrderDetail({ order, onClose, onStatusChange }: {
  order: Order;
  onClose: () => void;
  onStatusChange: (id: string, estado: Order['estado']) => void;
}) {
  const [saving, setSaving] = useState(false);
              ...
              <tbody>
                {order.items.map((item, i) => (
                  <tr key={i}>
                    <td>
                      <div>{item.nombre}</div>
                      <div className={styles.tdMuted} style={{ fontSize: '0.75rem' }}>{item.categoria}</div>
                    </td>
          ...
          <div className={styles.statusSection}>
            ...
            <div className={styles.statusBtns}>
              {ORDER_ESTADOS.map((e) => (
                <button
                  key={e}
                  className={`${styles.statusBtn} ${order.estado === e ? styles.statusBtnActive : ''}`}
                  onClick={() => handleStatus(e)}
                  disabled={saving || order.estado === e}
                >
                  {e}
                </button>
              ))}
...
export default function AdminDashboardClient() {
  ...
  useEffect(() => { setApiKey(sessionStorage.getItem('dg_api_key') ?? ''); }, []);
  ...
  const loadOrders = useCallback(async () => {
    setLoadingO(true);
    const url = filterEstado ? `/api/orders?estado=${filterEstado}` : '/api/orders';
    const res = await fetch(url);
        ...
        <nav className={styles.sidebarNav}>
          ...
          <button className={`${styles.navItem} ${tab === 'orders' ? styles.navItemActive : ''}`} onClick={() => setTab('orders')}>
            ...
            {orders.filter((o) => o.estado === 'pendiente').length > 0 && (
              <span className={styles.navBadge}>{orders.filter((o) => o.estado === 'pendiente').length}</span>
            )}
          </button>
        </nav>
          ...
          <>
            <div className={styles.pageHeader}>
              <div>
                ...
                <p className={styles.pageSubtitle}>{products.length} productos en total</p>
              </div>
              <button className={styles.btnPrimary} onClick={() => setShowNewForm(true)}>+ Nuevo producto</button>
            </div>

            {!apiKey && <ApiKeyPrompt onSet={setApiKey} />}

            <div className={styles.filters}>
              <input className={styles.input} placeholder="Buscar por nombre…" value={search} onChange={(e) => setSearch(e.target.value)} />
                ...
                <table className={styles.table}>
                  ...
                  <tbody>
                    {filteredProducts.length === 0 ? (
                      <tr><td colSpan={5} className={styles.emptyRow}>No hay productos</td></tr>
          ...
          <>
            <div className={styles.pageHeader}>
              <div>
                ...
                <p className={styles.pageSubtitle}>{orders.length} pedidos en total</p>
              </div>
              <button className={styles.btnSecondary} onClick={loadOrders}>↺ Refrescar</button>
            </div>

            <div className={styles.filters}>
              <select className={styles.input} value={filterEstado} onChange={(e) => setFilterEstado(e.target.value)}>
                <option value="">Todos los estados</option>
                {ORDER_ESTADOS.map((e) => <option key={e}>{e}</option>)}
              </select>
            </div>
                ...
                <table className={styles.table}>
                  ...
                  <tbody>
                    {filteredOrders.map((o) => (
                      <tr key={o._id}>
                        <td className={styles.tdMuted} style={{ fontFamily: 'monospace', fontSize: '0.75rem' }}>
                        ...
                        <td>
                          <button className={styles.editBtn} onClick={() => setSelectedOrder(o)}>Ver</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
        ...
        <div className={styles.modalOverlay} onClick={() => setDeleteId(null)}>
          <div className={styles.confirmModal} onClick={(e) => e.stopPropagation()}>
            ...
            <div className={styles.confirmBtns}>
              ...
              <button className={styles.deleteBtnConfirm} onClick={() => handleDelete(deleteId)} disabled={deleting}>
