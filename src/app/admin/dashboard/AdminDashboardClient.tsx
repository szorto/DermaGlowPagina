...
function OrderStatusBadge({ estado }: { estado: Order["estado"] }) {
  return (
    <span
      className={`${styles.badge} ${styles[`order_${estado.replace(" ", "_")}`]}`}
    >
      {estado}
    </span>
  );
          ...
          <div className={styles.grid2}>
            ...
            <Field label="Estado / Badge">
              <select
                ...
                value={form.estado ?? ""}
                onChange={(e) =>
                  set("estado", (e.target.value || null) as Product["estado"])
                }
              >
        ...
        <div className={styles.modalHeader}>
          <div>
            ...
            <p className={styles.orderMeta}>
              #{order._id.slice(-8).toUpperCase()} · {fmtDate(order.creadoEn)}
            </p>
          </div>
          <button className={styles.modalClose} onClick={onClose}>
            ✕
...
export default function AdminDashboardClient() {
  ...
  const filteredOrders = useMemo(
    () =>
      filterEstado
        ? allOrders.filter((o) => o.estado === filterEstado)
        : allOrders,
  ...
  async function handleMarkAllComplete() {
    ...
    if (!confirm(`¿Marcar ${targets.length} pedido(s) como completados?`))
      return;
    setMarkingAll(true);
    await Promise.all(
      targets.map((o) =>
    ...
    setAllOrders((prev) =>
      prev.map((o) =>
        targets.find((t) => t._id === o._id)
          ? { ...o, estado: "completado" as Order["estado"] }
          : o,
      ),
    );
    setMarkingAll(false);
  ...
  function handleStatusChange(id: string, estado: Order["estado"]) {
    ...
    if (selectedOrder?._id === id)
      setSelectedOrder((o) => (o ? { ...o, estado } : o));
  }
    ...
    <div className={styles.page}>
      <aside className={styles.sidebar}>
        ...
        <nav className={styles.sidebarNav}>
          <button
            className={`${styles.navItem} ${tab === "products" ? styles.navItemActive : ""}`}
          ...
          <button
            className={`${styles.navItem} ${tab === "orders" ? styles.navItemActive : ""}`}
        ...
        {tab === "products" && (
          <>
            ...
            <div className={styles.filters}>
              ...
              <button className={styles.btnSecondary} onClick={loadProducts}>
                ↺ Refrescar
              </button>
            </div>
            {loadingP ? (
                            ...
                            {p.precioNuevo != null ? (
                              <span>
                                <span className={styles.priceNew}>
                                  L {p.precioNuevo.toLocaleString()}
                                </span>{" "}
          ...
          <>
            <div className={styles.pageHeader}>
              ...
              <div className={styles.headerActions}>
                ...
                <button
                  ...
                  disabled={
                    markingAll ||
                    filteredOrders.every((o) => o.estado === "completado")
                  }
                >
            ...
            <div className={styles.statusTabs}>
              {(["", ...ORDER_ESTADOS] as const).map((e) => (
                <button
                  key={e}
                  className={`${styles.statusTab} ${filterEstado === e ? styles.statusTabActive : ""}`}
                  onClick={() => setFilterEstado(e)}
                >
                  {e === "" ? "Todos" : e.charAt(0).toUpperCase() + e.slice(1)}
                  <span className={styles.tabCountNeutral}>
                    {e === ""
                      ? allOrders.length
                      : allOrders.filter((o) => o.estado === e).length}
                  </span>
                </button>
              ))}
            </div>
