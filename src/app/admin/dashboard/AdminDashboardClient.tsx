...
interface OrderItem {
  ...
  precio: number; precioNuevo?: number; cantidad: number; subtotal: number;
}
interface Order {
...
function ProductForm({initial,onSave,onClose}:{initial?:Product;onSave:(p:Product)=>void;onClose:()=>void}) {
  ...
  async function handleSave() {
    ...
    const body={...form,precio:Number(form.precio),precioNuevo:form.precioNuevo?Number(form.precioNuevo):undefined,highlights:hlText.split('\n').map(s=>s.trim()).filter(Boolean)};
            ...
            <table className={styles.table}>
              ...
              <tbody>
                {order.items.map((item,i)=>(
                  <tr key={i}>
                    <td><div>{item.nombre}</div><div className={styles.tdMuted} style={{fontSize:'0.75rem'}}>{item.categoria}</div></td>
              ...
              {(['', ...ORDER_ESTADOS] as const).map(e=>(
                ...
                >
                  ...
                  <span className={styles.tabCountNeutral}>
                    {e===''?allOrders.length:allOrders.filter(o=>o.estado===e).length}
                  </span>
                </button>
              ))}
            </div>
